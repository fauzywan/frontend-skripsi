...
export async function downloadCsv(file) {
  ...
  const body = new FormData();
  body.append("file", file);
  const response = await fetch(`${API_BASE}/api/v1/download-csv`, { method: "POST", body });
  if (!response.ok) throw new Error(await parseError(response));
...
export function downloadPdf(result, counts, rows) {
  ...
  autoTable(doc, {
    ...
    body: [[metrics.accuracy, metrics.precision_macro, metrics.recall_macro, metrics.macro_f1].map((value) => (value == null ? "-" : Number(value).toFixed(4)))],
    headStyles: { fillColor: [23, 61, 51] },
  });
  ...
  autoTable(doc, {
    ...
    body: rows.slice(0, 100).map((row, index) => [index + 1, String(row.review_text || "-"), String(row.predicted_sentiment || "-")]),
