...
const nav = [
  ...
  ["visualization", "Visualisasi", "chart"],
  ["results", "Hasil Klasifikasi", "table"],
];
const labelKey = (value = "") => {
  const text = String(value).toLowerCase();
  return text.includes("posit") ? "positive" : text.includes("negat") ? "negative" : "neutral";
};
...
export default function App() {
  ...
  function getPdf() {
    try {
      downloadPdf(result, counts, rows);
    } catch (requestError) {
      window.alert(`PDF gagal dibuat: ${requestError.message}`);
    }
  }
      ...
      <div className="upload-entry">
        <header>
          ...
          <div className="entry-meta">
            <b>Sistem Analisis Sentimen</b>
            <span>Model Support Vector Machine</span>
          </div>
        </header>
        <main>
          <UploadPage onAnalyze={upload} loading={loading} error={error} />
        </main>
        <footer>© 2026 Na Willa · Aplikasi Analisis Sentimen untuk Penelitian Akademik</footer>
      </div>
    );
  ...
  return (
    <div className="workspace-app">
        ...
        <div className="header-main">
          ...
          <div className="header-actions">
            <div className="dataset-status">
              ...
              <div>
                <b>Dataset siap</b>
                <span>{result.filename || "Dataset ulasan"}</span>
              </div>
            </div>
            <button className="new-analysis" onClick={resetAnalysis}>
              <Icon name="upload" />
              <span>Dataset Baru</span>
            </button>
          </div>
        </div>
      ...
      <div className="workspace-content">
        <main>
          {page === "dashboard" && <DashboardPage {...props} />} {page === "visualization" && <VisualizationPage {...props} />} {page === "results" && <ResultsPage {...props} />}
        </main>
        <footer>© 2026 Na Willa · Aplikasi Analisis Sentimen untuk Penelitian Akademik</footer>
      </div>
    </div>
